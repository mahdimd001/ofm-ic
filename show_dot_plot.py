...
def compute_avg_from_log(file_path):
    # Data accumulators
    sums = defaultdict(int)
    counts = defaultdict(int)

    # Regex to extract the dictionary
    pattern = r"defaultdict\(.*?, ({.*?})\)"
    ...
    averages = {k: round(sums[k] / counts[k], 2) for k in sorted(sums)}
    for k, avg in averages.items():
        print(f"{k}: {avg}")

# Example usage:
compute_avg_from_log("/work/LAS/jannesar-lab/msamani/SuperSAM/logs/2025-06-09--20:14:49.366846_dataset[zh-plus-tiny-imagenet]_trainable[em]_epochs[50]_lr[5e-05]_bs[64]/extracted_lines.log")
