...
def get_optimizer_and_scheduler(params,lr=1e-5,weight_decay=0,scheduler=None):
    try:
        ...
        print(f'Error in get_optimizer_and_scheduler: {e}')
        raise Exception(f'Error in get_optimizer_and_scheduler: {e}. Please check the parameters and scheduler configuration.') 
...
def get_trainable_parameters(model,trainable):
    """samani cahnaged to support both sam and vit models"""
    try:
        if model.config.model_type.lower() == 'sam':
            ...
            if 'p' in trainable:
                trainable_params += list(model.vit.embeddings.parameters())
            return trainable_params
    except Exception as e:
        print(f'Error in get_trainable_parameters: {e}')
        raise Exception(f'Error in get_trainable_parameters: {e}. Please check the model type and trainable parameters.')
